...
export default function TextInputPanel({ show, disabled, onSubmit, onMic, onClickOutside }: TextInputPanelProps) {
  ...
  const playKeystroke = () => {
    if (throttleRef.current || volRef.current === 0) return;
    throttleRef.current = true;
    setTimeout(() => { throttleRef.current = false; }, 60);
    try {
      if (!audioRef.current) audioRef.current = new Audio('/efx/typewriter.mp3');
      const a = audioRef.current;
      a.currentTime = 0;
      a.volume = 0.25 * volRef.current;
      a.play().catch(() => {});
    } catch {}
  };
      ...
      {show && (
        ...
        >
          <form
            onSubmit={(e) => {
              e.preventDefault();
              if (value.trim() && !disabled) { onSubmit(value.trim()); setValue(''); }
            }}
            className="flex items-center gap-2 bg-surface-dark/95 border border-surface rounded-xl px-3 py-2 shadow-2xl"
          >
            <input
              type="text"
              value={value}
              onChange={(e) => { playKeystroke(); setValue(e.target.value); }}
