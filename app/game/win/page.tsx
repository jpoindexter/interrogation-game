      ...
      <div className="max-w-2xl mx-auto px-6 sm:px-8 pb-6 sm:pb-8 relative z-10">
        ...
        <motion.div className="mt-10 space-y-3" initial={{ opacity: 0 }} animate={revealStep >= 5 ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 1.5 }}>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={() => { sessionStorage.removeItem('gameResult'); const next = difficulty === 'easy' ? 'medium' : difficulty === 'medium' ? 'hard' : 'expert'; router.push(`/game?setting=${encodeURIComponent(caseSetting)}&difficulty=${next}`); }} className="px-8 py-4 bg-accent text-white font-bold rounded-sm hover:bg-accent-hover transition-colors text-center">TRY HARDER</button>
            <button onClick={() => { sessionStorage.removeItem('gameResult'); router.push('/cases'); }} className="px-8 py-4 bg-gold text-black font-bold rounded-sm hover:bg-gold-hover transition-colors text-center">NEW CASE</button>
            <button onClick={() => router.push('/leaderboard')} className="px-8 py-4 bg-surface text-foreground font-bold rounded-sm hover:bg-surface-hover transition-colors text-center">LEADERBOARD</button>
            <button onClick={async () => { if (!breakdown) return; const url = typeof window !== 'undefined' ? window.location.origin : ''; const text = [`\ud83d\udd0d INTERROGATION \u2014 Case #${result.caseData.case_number}`, `Cracked ${result.caseData.suspect_name} in ${formatTime(result.timeElapsed)}`, `Score: ${breakdown.finalScore.toLocaleString()} | Rating: ${getRating(breakdown.finalScore)}`, `Can you beat my score?`, url].join('\n'); const outcome = await shareResult(text); if (outcome === 'copied') { setShareLabel('COPIED!'); setTimeout(() => setShareLabel('SHARE'), 2000); } }} className="px-8 py-4 bg-surface text-foreground font-bold rounded-sm hover:bg-surface-hover transition-colors text-center">{shareLabel}</button>
            <button onClick={() => setShowTranscript(true)} className="px-8 py-4 bg-surface text-foreground font-bold rounded-sm hover:bg-surface-hover transition-colors text-center">TRANSCRIPT</button>
          </div>
        </motion.div>
      </div>
