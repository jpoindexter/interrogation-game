...
export default function InitialsEntry({ score, onSubmit, onViewScore, onViewLeaderboard }: InitialsEntryProps) {
  ...
  const cycle = useCallback((slot: number, dir: number) => {
    setInitials(prev => {
      const next = [...prev];
      const idx = CHARS.indexOf(next[slot]);
      next[slot] = CHARS[(idx + dir + 26) % 26];
      return next;
    });
  }, []);
  ...
  const handleKey = useCallback((e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowUp') cycle(activeSlot, 1);
      ...
      setInitials(prev => { const n = [...prev]; n[activeSlot] = letter; return n; });
  ...
  return (
    ...
    >
      <motion.div
        ...
        tabIndex={0}
        onKeyDown={handleKey}
        className="text-center outline-none px-6"
      ...
      >
        {/* HIGH SCORE flash */}
        <motion.p
          className="text-gold text-sm uppercase tracking-[0.4em] mb-2"
        ...
        {/* Score number */}
        <motion.p
          className="text-5xl sm:text-7xl font-bold text-gold tabular-nums mb-6"
          ...
          {!submitted ? (
            ...
            >
              ...
              <div className="flex justify-center gap-4 sm:gap-6 mb-8">
                {initials.map((char, i) => (
                  <div key={i} className="flex flex-col items-center gap-1">
                    <button
                      onClick={() => { setActiveSlot(i); cycle(i, 1); }}
                    ...
                    >
                      ...
                      {i === activeSlot && (
                        <motion.span
                          className="absolute bottom-1 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-gold"
                          animate={{ opacity: [1, 0] }}
              ...
              <motion.button
                ...
                className="px-10 py-4 bg-gold text-black font-bold text-sm uppercase tracking-[0.2em] rounded-sm hover:bg-gold-hover transition-colors"
            ...
            >
              <p className="text-sm text-gray-400 uppercase tracking-[0.2em] mb-6">
                {initials.join('')} — Recorded
              </p>

              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <motion.button
                  onClick={onViewLeaderboard}
                  className="px-8 py-4 bg-gold text-black font-bold text-sm uppercase tracking-[0.2em] rounded-sm hover:bg-gold-hover transition-colors"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.97 }}
                >
                  View Leaderboard
                ...
                <motion.button
                  onClick={onViewScore}
                  className="px-8 py-4 bg-surface text-foreground font-bold text-sm uppercase tracking-[0.2em] rounded-sm hover:bg-surface-hover transition-colors"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.97 }}
                >
                  Back to Score
